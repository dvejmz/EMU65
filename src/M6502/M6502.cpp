...
#include "../../include/M6502/Tables.h"
...
/*************************************************************/
/* TODO -- Transfer ownership of the Cpu instance created by Aim65 */
  ...
  while(RunCycles>0)
  {
    ...
    if(R->Trace)
    {
      //if(!Debug6502(R)) return(RunCycles);
        if(!cpu->UpdateDbgStatus()) return(RunCycles);
  ...
  for(;;)
  {
    ...
    if(R->Trace)
    {
      //if(!Debug6502(R)) return(R->PC.W);
        if(!cpu->UpdateDbgStatus()) return(R->PC.W);
    }
#endif
...
/** SECTION DEFINED BY DAVID J SEQUERO *************************/
/** All the code written from this point, except the function **/
/** prototypes and function comments, does not belong to      **/
/** Marat Fayzullin.                                          **/
